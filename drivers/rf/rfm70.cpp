...
//! \par release notes
//! V1.03 2013-04-20) 
//! Felix Eisele make some changes
//!
//! V1.02 (2011-12-31)
//! Arduino added
//!
...
unsigned char rfm70::SPI_RW( unsigned char value ){
	...
	*/
	unsigned char result = ssp->SSPSend(value);
  return result;
}
...
void rfm70::register_write( unsigned char reg, unsigned char value ){
   ...
   this->chipSelect->SetValue(false);                // CSN low, init SPI transaction
   (void)rfm70::SPI_RW( reg );    // select register
   (void)rfm70::SPI_RW( value );  // ..and write value to it..
   this->chipSelect->SetValue(true);                // CSN high again
}
...
unsigned char rfm70::register_read( unsigned char reg ){
   ...
   this->chipSelect->SetValue(false);               // CSN low, initialize SPI communication...
   (void)rfm70::SPI_RW( reg );   // Select register to read from..
   value = rfm70::SPI_RW( 0 );   // ..then read register value
   this->chipSelect->SetValue(true);                // CSN high, terminate SPI communication
...
void rfm70::buffer_read( 
   ...
   this->chipSelect->SetValue(false);                   // Set CSN 0
   ...
   this->chipSelect->SetValue(true);                    // Set CSN high again
}
...
void rfm70::buffer_write( 
   ...
   this->chipSelect->SetValue(false);                    // Set CSN low, init SPI tranaction
...
void rfm70::init( void ){
   ...
   this->ce->SetValue(false);
  this->chipSelect->SetValue(false);
